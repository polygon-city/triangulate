...
//   var points2D = points3dto2d(_.clone(points)).points;
...
//
//   _.each(_.flatten(contours), function(vertex, index) {
...
//   var faces = _.map(_.chunk(triangleVerts, 6), function(triVerts) {
...
//   var points2D = points3dto2d(_.clone(points)).points;
//   var faces = _.chunk(earcut(_.flatten(points2D)), 3);
//   return faces;
// };
...
var triangulatePnlTri = function(points) {
  ...
  var contour2D = _.map(cPoints2D.points, function(point) {
    var point2D = {
      x: point[0],
      y: point[1]
    };
